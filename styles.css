...
async function loadAll() {
  ...
  $('truthPackSelect').innerHTML = '<option value="">Choose game level</option>' + packs.map(p => `<option value="${p.id}">${escapeHtml(p.gameTitle || 'The Serpent’s Breath')} — ${escapeHtml(p.levelLabel || p.difficulty || p.title)}</option>`).join('');
...
function phaseLabel(phase) {
  return ({ lobby: 'Lobby', briefing: 'Briefing', investigation: 'Investigation', accusation: 'Escape Open', accusation_locked: 'Escape Locked', revealed: 'Revealed' })[phase] || phase;
}
...
function fullPlayerName(item) {
  ...
  return full || item.fullName || item.playerName || item.name || 'Archaeologist';
}
...
function hostPrimaryAction(s) {
  ...
  if (s.status === 'started') return `<button class="danger" onclick="revealTemple('${s.sessionCode}')">Trigger Reveal</button>`;
...
function sponsorAdHtml(s) {
  ...
  const options = [
    ...
    ['before_final', 'Before Final Escape'],
    ['temple_closed', 'Temple Closed Screen']
  ];
  ...
  return `<div class="adManagerBox">
    <p class="mini">Automated sponsor popups appear at natural breaks and are dismissible. They will not interrupt Field Notes or active checkpoint answering.</p>
    <div class="row">
    ...
    <article class="tableCard">
      <div class="row">
        <div>
          ...
          <p class="mini">${escapeHtml(s.truthPackTitle)} · ${escapeHtml(s.difficultyLabel || s.difficulty || '')} · ${escapeHtml(eventLine(s))} · ${escapeHtml(eventTypeLabel(s))} · Archaeologist Mode · Unified Evidence</p>
        </div>
        <div class="statusPills">
          ...
          <span class="pill">${s.players.length} Archaeologists</span>
      ...
      <div class="actions" style="margin-top:12px">
        ...
        <button class="danger" onclick="revealTemple('${s.sessionCode}')">Reveal Temple</button>
      ...
      <div class="card" style="box-shadow:none">
        ...
        <p class="mini">Current objective: ${escapeHtml(s.currentRound?.objective || 'Waiting to start the temple.')}</p>
      ...
      <div class="grid">
        ...
        <div class="card" style="box-shadow:none">
          <h3>Connected Archaeologists</h3>
          <div>${playerList}</div>
      ...
      <div class="grid">
        ...
        <div class="card" style="box-shadow:none"><h3>Escapes</h3>${submissionHtml(s.submissions, s.accusation)}</div>
      </div>
...
window.startTable = async code => {
  try {
    ...
    await api(`/api/sessions/${code}/message`, {
      method: 'POST',
      body: { title: 'Opening Briefing', text: session?.openingNarration || 'Archaeologists, the temple is beginning.', kind: 'opening' }
    });
    hostNotify('Briefing has started. Players in the waiting room will enter the temple setup.');
...
window.revealTemple = async code => { try { await api(`/api/sessions/${code}/reveal`, { method: 'POST' }); hostNotify('Reveal has been triggered.'); } catch (err) { hostNotify(err.message || 'Could not reveal temple.', 'error'); } };
