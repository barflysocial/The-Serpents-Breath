...
const sessionCode = (params.get('session') || '').toUpperTemple();
...
function renderSession(info) {
  sessionInfo = info;
  $('sessionTitle').textContent = info.tableName || 'The Serpent’s Breath Check-In';
  $('sessionMeta').textContent = `${info.truthPackTitle || 'The Serpent’s Breath'} · ${info.eventDateLabel || 'Date TBD'} · ${info.eventTimeLabel || 'Time TBD'}`;
