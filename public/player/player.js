...
const DEMO_PLAY_ACCESS_CODE = 'SERPENT';
...
if ($('demoPlayNowBtn')) $('demoPlayNowBtn').onclick = demoPlayNow;



async function demoPlayNow() {
  const msg = $('demoPlayMessage') || $('joinError');
  if (msg) msg.textContent = 'Starting demo escape room...';
  if ($('accessCode')) $('accessCode').value = DEMO_PLAY_ACCESS_CODE;
  termsAcceptedForCurrentAction = true;
  try {
    await join(DEMO_PLAY_ACCESS_CODE, msg);
  } finally {
