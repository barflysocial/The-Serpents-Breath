...
if ($('enterChamberBtn')) $('enterChamberBtn').onclick = enterChamberFromSummary;
...
function levelSummaryForRound(round, index = 0) {
  const summaries = [
    {
      ...
      where: 'The sealed entry chamber below Chichén Itzá, where the first wall of glyphs blocks the passage.',
      test: 'Torch control, basic translation, and recognizing that carved symbols are instructions.',
      goal: 'Use the torch briefly, locate the first breath glyph, and identify the mark that opens the entry seal.',
      danger: 'Leaving the torch burning wastes oxygen before the real temple begins.',
      mechanic: 'Drag the torch across the black chamber view to reveal hidden glyphs and then choose the correct translation.'
    },
    {
      title: 'Level 2: The Shadow Wall',
      where: 'A narrow corridor lined with serpent carvings and broken stone seals.',
      test: 'Light, shadow direction, and matching a projected serpent shape to the correct seal.',
      goal: 'Find the serpent shadow that points to the safe carving.',
      danger: 'Wrong seals cost oxygen and force the chamber to reset.',
      mechanic: 'Move the torch to study how symbols and shadows align before selecting a path.'
    },
    {
      title: 'Level 3: The Balance Chamber',
      where: 'A pressure-floor room with stone weights, carved plates, and a hidden counterbalance gate.',
      test: 'Mass, balance, torque, and reading number-like glyph clusters.',
      goal: 'Balance the correct stones so the gate lifts without wasting oxygen.',
      danger: 'Stepping on the wrong pressure plate drains oxygen and locks the room for a moment.',
      mechanic: 'Reveal floor marks with the torch, compare weight symbols, and choose the balanced plate.'
    },
    {
      title: 'Level 4: The Three Paths',
      where: 'A junction where three tunnels breathe differently through the temple.',
      test: 'Airflow, echo, direction, and matching wall/floor/ceiling carvings.',
      goal: 'Choose the living path that leads toward fresh air and away from dead-end chambers.',
      danger: 'Wrong turns burn time, oxygen, and momentum.',
      mechanic: 'Use short torch sweeps to inspect tunnel carvings, airflow marks, and echo symbols.'
    },
    {
      title: 'Level 5: The Serpent Gate',
      where: 'The final gate beneath the temple, where the serpent, star, breath, and shadow symbols converge.',
      test: 'Combining every translation and physics clue from the previous chambers.',
      goal: 'Open the final oxygen gate and escape before the timer reaches zero.',
      danger: 'The final choice locks the route. A rushed guess can trap the archaeologist inside.',
      mechanic: 'Use the torch carefully to connect wall, floor, and ceiling clues into one final escape sequence.'
    }
  ];
  ...
  return {
    ...
    intro: round?.dialogue || round?.objective || 'Read the chamber, conserve oxygen, and solve the level before moving deeper into the temple.'
  };
}
...
function openLevelSummary(round, next) {
  ...
  if ($('levelSummaryIntro')) $('levelSummaryIntro').textContent = summary.intro;
