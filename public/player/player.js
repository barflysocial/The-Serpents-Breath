...
function renderAccuse() {
  if ($('finalDramaBox')) $('finalDramaBox').classList.add('hidden');
  if ($('accuseStatus')) $('accuseStatus').textContent = 'Final escape is handled by the Serpent Gate chamber. Complete the chamber puzzles to escape.';
  show('accuseFormWrap', false);
  if ($('submitAccuseBtn')) $('submitAccuseBtn').disabled = true;
  if ($('submitAccuseBtn')) $('submitAccuseBtn').textContent = 'Complete the Serpent Gate';
  if ($('accuseQuestions')) $('accuseQuestions').innerHTML = '<p class="muted">No multiple-choice gate questions are used in this escape room.</p>';
  if ($('accuseResult')) $('accuseResult').textContent = '';
}
...
function saveQuestionAnswer(input) {
  if ($('accuseResult')) $('accuseResult').textContent = 'Multiple-choice gate questions are disabled for this escape room.';
  return Promise.resolve();
}

async function submitEscape() {
  if ($('accuseResult')) $('accuseResult').textContent = 'Final escape is completed through the Serpent Gate chamber.';
}

function nextUnansweredCheckpointQuestion() { return null; }
function renderCheckpointPopup() {
...
async function submitCheckpointPopup() { return; }
window.openCheckpointQuestion = function() { return; };
  ...
  if (round && elapsed >= Number(round.startSec || 0) + 20) {
    ...
    if (!ack.messages.includes(id)) {
      ...
      enqueueDialog({
        ...
        text: round.breakingUpdate || `New information is now active for ${round.title || 'this chamber'}. Review the latest evidence before the next gate opens.`,
        ackType: 'message',
        ackValue: id
      });
    }
...
function inspectCountdown(next) {
  ...
  if (secsUntil > 0 && secsUntil <= 10) {
    $('countdownMeta').textContent = 'Gate Opening';
    $('countdownTitle').textContent = `Next: ${upcoming.title}`;
    $('countdownReview').textContent = next.currentRound?.countdownReview || next.currentRound?.objective || 'Review your discoveries and get ready for the next chamber.';
    $('countdownNumber').textContent = secsUntil;
    $('countdownNext').textContent = `${upcoming.dialogue || upcoming.objective || 'A new chamber is about to open.'}`;
