...
function normalizeAccessCode(value) {
  ...
  if (!cleaned) return '';
  if (cleaned.startsWith('SERPFREE')) return `SERP-FREE-${cleaned.slice(8)}`.slice(0, 24);
  if (cleaned.startsWith('SERP')) return `SERP-${cleaned.slice(4)}`.slice(0, 24);
...
const DEFAULT_DEMO_ACCESS_CODE = 'SERPENT';
...
function sanitizeArchaeologistNotes(notes = {}) {
...
function makeDisplayName(firstName, lastName, fallback = 'Archaeologist') {
  const first = String(firstName || '').trim();
  const last = String(lastName || '').trim();
  if (first && last) return `${first} ${last.charAt(0).toUpperCase()}.`;
  if (first) return first;
  const fb = String(fallback || '').trim();
  return fb || 'Archaeologist';
}

function fullName(firstName, lastName, fallback = 'Archaeologist') {
  const name = `${String(firstName || '').trim()} ${String(lastName || '').trim()}`.trim();
  return name || String(fallback || '').trim() || 'Archaeologist';
}
...
function servePlayerWithRichPreview(req, res) {
  ...
  const imageUrl = `${origin}/assets/serpents-breath-title-bg.png`;
...
function getPack(sessionOrId) {
  ...
  return truthPacks[id] || truthPacks['serpents-breath-escape'] || truthPacks['serpents-breath-escape'] || truthPacks.demo;
}
...
function currentLevelSummaryPauseSec(session) {
  if (!session) return 0;
  const saved = Math.max(0, Math.floor(Number(session.levelSummaryPausedTotalSec || 0)));
  if (session.levelSummaryPaused && session.levelSummaryPausedAt) {
    return saved + Math.max(0, Math.floor((Date.now() - Number(session.levelSummaryPausedAt || 0)) / 1000));
  }
  return saved;
}

function settleLevelSummaryPause(session) {
  if (!session || !session.levelSummaryPaused) return;
  session.levelSummaryPausedTotalSec = currentLevelSummaryPauseSec(session);
  session.levelSummaryPaused = false;
  session.levelSummaryPausedAt = null;
}

function pauseForLevelSummary(session, roundId = '') {
  if (!session || session.status !== 'started' || session.revealed) return false;
  normalizeOxygenState(session);
  settleTorchPenalty(session);
  session.torchActive = false;
  session.torchStartedAt = null;
  if (!session.levelSummaryPaused) {
    session.levelSummaryPaused = true;
    session.levelSummaryPausedAt = Date.now();
  }
  session.levelSummaryRoundId = String(roundId || session.levelSummaryRoundId || '').slice(0, 80);
  return true;
}
...
function computeElapsedSec(session) {
  ...
  normalizeOxygenState(session);
  const pausedSec = currentLevelSummaryPauseSec(session);
  if (!session.startedAt) return Math.min(GAME_TOTAL_SEC, Math.max(0, Math.floor(Number(session.elapsedAtPause || 0) + currentOxygenPenaltySec(session))));
  const baseElapsed = Math.max(0, Math.floor((Date.now() - session.startedAt) / 1000) - pausedSec);
  return Math.min(GAME_TOTAL_SEC, baseElapsed + currentOxygenPenaltySec(session));
}
...
function normalizeOxygenState(session) {
  if (!session) return { torchActive: false, oxygenPenaltySec: 0, torchStartedAt: null };
  session.oxygenPenaltySec = Math.max(0, Math.floor(Number(session.oxygenPenaltySec || 0)));
  session.torchActive = Boolean(session.torchActive);
  session.torchStartedAt = session.torchStartedAt ? Number(session.torchStartedAt) : null;
  return session;
}

function currentOxygenPenaltySec(session) {
  normalizeOxygenState(session);
  let penalty = Number(session.oxygenPenaltySec || 0);
  if (session.status === 'started' && session.torchActive && session.torchStartedAt) {
    // Torch burns usable oxygen faster. One real second with torch on costs one extra oxygen second.
    penalty += Math.max(0, Math.floor((Date.now() - session.torchStartedAt) / 1000));
  }
  return Math.max(0, Math.floor(penalty));
}

function settleTorchPenalty(session) {
  normalizeOxygenState(session);
  if (session.torchActive && session.torchStartedAt) {
    session.oxygenPenaltySec = currentOxygenPenaltySec(session);
    session.torchStartedAt = Date.now();
  }
  return session.oxygenPenaltySec;
}

function addOxygenPenalty(session, seconds = 0, reason = 'penalty') {
  normalizeOxygenState(session);
  settleTorchPenalty(session);
  const add = Math.max(0, Math.floor(Number(seconds || 0)));
  if (!add) return 0;
  session.oxygenPenaltySec += add;
  session.oxygenPenaltyLog = Array.isArray(session.oxygenPenaltyLog) ? session.oxygenPenaltyLog : [];
  session.oxygenPenaltyLog.unshift({ id: nanoid(8), seconds: add, reason, createdAt: Date.now() });
  session.oxygenPenaltyLog = session.oxygenPenaltyLog.slice(0, 50);
  return add;
}

function oxygenStats(session) {
  const penalty = currentOxygenPenaltySec(session);
  return {
    torchActive: Boolean(session?.torchActive),
    oxygenPenaltySec: penalty,
    oxygenBurnRate: session?.torchActive ? 2 : 1,
    oxygenNote: session?.torchActive ? 'Torch on: clues are visible, oxygen drains faster.' : 'Torch off: oxygen drains normally, hidden carvings are harder to read.',
    oxygenPenaltyLog: Array.isArray(session?.oxygenPenaltyLog) ? session.oxygenPenaltyLog.slice(0, 10) : []
  };
}
...
function getBadge(score, total) {
  if (score >= total) return 'Senior Archaeologist';
  if (score >= 8) return 'Archaeologist';
  if (score >= 6) return 'Junior Archaeologist';
  if (score >= 4) return 'Temple Explorer';
  return 'Training Archaeologist';
}
...
function clientState(session) {
  ...
  return {
    ...
    difficulty: pack.difficulty || 'Temple Explorer',
    difficultyLabel: pack.difficultyLabel || '30-MINUTE OXYGEN ESCAPE',
    openingNarration: pack.openingNarration || '',
    revealScript: pack.revealScript || '',
    accusation: sanitizeAccusation(pack.accusation),
    mode: 'Escape Room Mode',
    evidenceModel: 'Unified Temple Clues',
    phase,
    status: session.status,
    elapsedSec,
    remainingSec,
    levelSummaryPaused: Boolean(session.levelSummaryPaused),
    levelSummaryPausedRoundId: session.levelSummaryRoundId || '',
    levelSummaryPausedSec: currentLevelSummaryPauseSec(session),
    ...oxygenStats(session),
    briefingElapsedSec,
...
function createSession({ tableName = 'Table 1', truthPackId = 'serpents-breath-escape', playerCap = 25, eventDate = '', eventTime = '', eventDurationMinutes = 45, eventStatus = 'open', eventType = 'paid', ticketPrice = '', eventPrice = '', sharedAccessCode = '', allowLateCheckIn = false, scheduledStartAt = 0, autoStartEnabled = true } = {}) {
  let code = makeCode();
  ...
  const session = {
    ...
    code,
    tableName,
    truthPackId: truthPacks[truthPackId] ? truthPackId : 'serpents-breath-escape',
    ...
    oxygenPenaltySec: 0,
    oxygenPenaltyLog: [],
    levelSummaryPaused: false,
    levelSummaryPausedAt: null,
    levelSummaryPausedTotalSec: 0,
    levelSummaryRoundId: '',
    createdAt: Date.now(),
    players: new Map(),
    hostMessages: [],
...
function prepareDemoSession(session) {
  ...
  session.tableName = session.tableName || 'Training Level 1 — Chichén Itzá';
  session.truthPackId = session.truthPackId || 'serpents-breath-escape';
  ...
  if (!session.hostMessages.length) {
    ...
    addHostMessage(session, {
      ...
      text: pack.openingNarration || 'The Serpent’s Breath Training Level 1 is live. Review the evidence as it unlocks and submit your final escape sequence before oxygen runs out.',
      kind: 'opening'
    });
  }
  return session;
...
function getOrCreateDemoSession() {
  ...
  if (!session) {
    session = createSession({
      tableName: 'Training Level 1 — Chichén Itzá',
      truthPackId: 'serpents-breath-escape',
      playerCap: 100,
      eventStatus: 'private',
...
function createFreshDemoSession() {
  ...
  // Demo Play Now should bypass the host/lobby/briefing controls and begin immediately.
  ...
  session.torchActive = false;
  session.torchStartedAt = null;
  session.oxygenPenaltySec = 0;
  session.oxygenPenaltyLog = [];
  session.revealed = false;
  session.results = [];
...
function revealSession(session, reason = 'auto') {
  ...
  if (!alreadyHasReveal) {
    addHostMessage(session, {
      title: 'Temple Sealed',
      text: pack.revealScript || 'Archaeologists, the case is closed.',
      kind: 'reveal'
    });
  }
  return true;
  ...
  if (!alreadyHasMessage) {
    addHostMessage(session, {
      ...
      text: reason === 'scheduled'
        ? 'The scheduled start time has arrived. Checked-in players are moving from the waiting room into the temple backstory.'
        : 'The host started the temple backstory. Checked-in players are moving from the waiting room into the temple backstory.',
      kind
    });
  }
  return true;
...
function startGameFromBriefing(session, startedAt = Date.now()) {
  ...
  if (!alreadyHasStart) {
    addHostMessage(session, {
      title: 'Escape Timer Started',
      text: 'The five-minute temple backstory is complete. The 30-minute investigation timer has begun. Open the investigation apps and follow the evidence.',
...
app.get('/api/truth-packs', (_req, res) => {
  const order = ['training','explorer','field','senior','master'];
  const packs = Object.values(truthPacks)
    .filter(p => p.gameId === 'serpents-breath' || p.id === 'serpents-breath-escape')
    .sort((a, b) => {
  ...
  res.json(packs.map(p => ({
    id: p.id,
    gameId: p.gameId || 'serpents-breath',
    gameTitle: p.gameTitle || 'The Serpent’s Breath',
    levelId: p.levelId || '',
  ...
  const items = Array.from(sessions.values())
    ...
    .map(session => {
      ...
      return {
        sessionCode: session.code,
        tableName: session.tableName,
        mystery: pack.gameTitle || 'The Serpent’s Breath',
        mysteryTitle: pack.title || pack.gameTitle || 'The Serpent’s Breath',
        venue: pack.venue || 'Mid City Chichén Itzá • Mexico',
...
app.post('/api/rsvps/lookup', async (req, res) => {
  ...
  if (demo) {
    const session = getOrCreateDemoSession();
    return res.json({ ok: true, demoMode: true, session: publicCheckinSession(session), rsvp: { firstName: 'Demo', displayName: 'Demo Archaeologist', accessCode: demoAccessCode, status: 'Demo' }, accessCode: demoAccessCode });
  }
...
app.post('/api/checkins', async (req, res) => {
  ...
  if (!rsvp && isFreeEvent) {
    rsvp = {
      id: nanoid(8), firstName: 'Guest', lastName: 'Archaeologist', instagram: '', contact: '', status: 'checked-in-walkup', paid: false,
...
app.post('/api/access/join', async (req, res) => {
  ...
  if (isDemoAccessCode(lookupValue || accessCode)) {
    ...
    const lastName = requestLastName || 'Archaeologist';
    const instagram = requestInstagram;
    const displayName = makeDisplayName(firstName, lastName, 'Demo Archaeologist');
    const legalName = fullName(firstName, lastName, 'Demo Archaeologist');
    const session = createFreshDemoSession();
...
app.get('/api/sessions/:code/notes/:playerId', (req, res) => {
  ...
  res.json({ ok: true, notes: sanitizeArchaeologistNotes(player.archaeologistNotes || {}) });
});
...
app.post('/api/sessions/:code/notes', async (req, res) => {
  ...
  const player = session.players.get(playerId);
  if (!player) return res.status(404).json({ error: 'Player not found in this session.' });
  player.archaeologistNotes = sanitizeArchaeologistNotes(req.body.notes || {});
  player.notesUpdatedAt = Date.now();
  session.players.set(playerId, player);
  await saveSession(session);
  res.json({ ok: true, notes: player.archaeologistNotes, updatedAt: player.notesUpdatedAt });
});

app.post('/api/sessions/:code/level-summary/pause', async (req, res) => {
  const session = getSessionOr404(req.params.code, res);
  if (!session) return;
  const roundId = String(req.body.roundId || '');
  pauseForLevelSummary(session, roundId);
  await saveSession(session);
  res.json({ ok: true, state: clientState(session) });
  broadcast(session.code);
});

app.post('/api/sessions/:code/level-summary/resume', async (req, res) => {
  const session = getSessionOr404(req.params.code, res);
  if (!session) return;
  settleLevelSummaryPause(session);
  session.levelSummaryRoundId = '';
  await saveSession(session);
  res.json({ ok: true, state: clientState(session) });
  broadcast(session.code);
});

app.post('/api/sessions/:code/chamber/complete', async (req, res) => {
  const session = getSessionOr404(req.params.code, res);
  if (!session) return;
  if (session.status !== 'started') return res.status(409).json({ error: 'The chamber can only advance after the game has started.' });

  const pack = loadTruthPack(session.truthPackId);
  const { rounds, current } = currentRoundFor(session, pack);
  const solvedRoundId = String(req.body.roundId || '');
  const currentIndex = rounds.findIndex(r => r.id === current?.id);
  if (currentIndex < 0) return res.status(400).json({ error: 'Current chamber could not be found.' });
  if (solvedRoundId && current?.id && solvedRoundId !== current.id) {
    return res.status(409).json({ error: 'That solved chamber is not the active chamber.' });
  }

  const next = rounds[currentIndex + 1];
  session.solvedChambers = Array.isArray(session.solvedChambers) ? session.solvedChambers : [];
  if (current?.id && !session.solvedChambers.includes(current.id)) session.solvedChambers.push(current.id);
  ...
  if (!next) {
    revealSession(session, 'escape_complete');
    await saveSession(session);
    res.json({ ok: true, complete: true, state: clientState(session) });
    broadcast(session.code);
    return;
  }

  const targetElapsed = Math.max(Number(next.startSec || 0), computeElapsedSec(session));
  const penalty = currentOxygenPenaltySec(session);
  const paused = currentLevelSummaryPauseSec(session);
  session.startedAt = Date.now() - Math.max(0, targetElapsed - penalty + paused) * 1000;
  session.elapsedAtPause = 0;

  // Pause immediately for the new chamber briefing. The player must press Continue: Enter Chamber before oxygen resumes.
  pauseForLevelSummary(session, next.id);

  await saveSession(session);
  res.json({ ok: true, nextRoundId: next.id, state: clientState(session) });
  broadcast(session.code);
});

app.post('/api/sessions/:code/torch', async (req, res) => {
  const session = getSessionOr404(req.params.code, res);
  if (!session) return;
  if (session.status !== 'started') return res.status(409).json({ error: 'Torch can only be used after the oxygen timer starts.' });
  if (session.levelSummaryPaused) return res.status(409).json({ error: 'Torch is unavailable during the chamber briefing.' });
  normalizeOxygenState(session);
  const active = Boolean(req.body.active);
  if (active && !session.torchActive) {
    session.torchActive = true;
    session.torchStartedAt = Date.now();
  } else if (!active && session.torchActive) {
    session.oxygenPenaltySec = currentOxygenPenaltySec(session);
    session.torchActive = false;
    session.torchStartedAt = null;
  }
...
app.post('/api/sessions/:code/oxygen-penalty', async (req, res) => {
  const session = getSessionOr404(req.params.code, res);
  if (!session) return;
  const seconds = Math.max(0, Math.min(300, Number(req.body.seconds || 0)));
  const reason = String(req.body.reason || 'Path penalty').slice(0, 160);
  addOxygenPenalty(session, seconds, reason);
  await saveSession(session);
  res.json({ ok: true, state: clientState(session) });
  broadcast(session.code);
});
...
app.post('/api/sessions/:code/help', async (req, res) => {
  ...
  const help = { id: nanoid(8), playerId, playerName: player?.name || 'Unknown Archaeologist', text, createdAt: Date.now(), status: 'open' };
...
app.post('/api/sessions/:code/answer', async (req, res) => {
  ...
  // Wrong checkpoint/path choices cost oxygen time. Correct answers do not add a penalty.
  for (const [qid, selectedId] of Object.entries(answers)) {
    const q = findQuestion(pack, qid);
    if (q && q.stage === 'checkpoint' && selectedId !== q.correctOptionId) {
      const key = `${playerId}:${qid}:${selectedId}`;
      session.appliedPathPenalties = Array.isArray(session.appliedPathPenalties) ? session.appliedPathPenalties : [];
      if (!session.appliedPathPenalties.includes(key)) {
        addOxygenPenalty(session, Number(q.wrongPenaltySec || 45), `Wrong turn: ${q.prompt}`);
        session.appliedPathPenalties.push(key);
      }
    }
  }
  await saveSession(session);
...
app.post('/api/sessions/:code/accuse', async (req, res) => {
  ...
  if (!session) return;
  const phase = computePhase(session);
  if (phase !== 'accusation') return res.status(409).json({ error: 'Final escape sequence is not open right now.' });
  const playerId = String(req.body.playerId || '');
  const player = session.players.get(playerId);
  if (!player) return res.status(404).json({ error: 'Player not found in this session.' });
  const pack = getPack(session);
  ...
  if (questions.length && missing.length) {
    return res.status(400).json({ error: `Please answer all ${questions.length} escape questions before submitting.` });
  }
  const submission = upsertSubmission(session, {
    id: nanoid(8),
    playerId,
...
async function startServer() {
  ...
  server.listen(PORT, () => {
    console.log(`Escape Room Mode Mystery App running on port ${PORT}`);
