...
function prepareDemoSession(session) {
  ...
  session.truthPackId = 'serpents-breath-escape';
  ...
  if (!session.hostMessages.length) {
    ...
    addHostMessage(session, {
      ...
      text: pack.openingNarration || 'The Serpent’s Breath Training Level 1 is live. Review the evidence as it unlocks and submit your accusation before time runs out.',
      kind: 'opening'
    });
  }
  return session;
  ...
  const items = Array.from(sessions.values())
    ...
    .map(session => {
      ...
      return {
        ...
        venue: pack.venue || 'Mid City Chichén Itzá • Baton Rouge, Louisiana',
